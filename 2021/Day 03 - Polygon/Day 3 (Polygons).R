...
milanP <- ggplot() +
  geom_sf(data=milan, aes(geometry=geometry), fill=colors[4], color=colors[4], size=1.25) +
  theme_void()+
  theme(plot.margin = margin(2, 0, 0, 0, unit="cm"))

nycP <- ggplot() +
  geom_sf(data=nyc, aes(geometry=geometry), fill=colors[5], color=colors[5], size=1) +
  theme_void() +
  theme(plot.margin = margin(2, 0, 0, 0, unit="cm"))

parisP <- ggplot() +
  geom_sf(data=paris, aes(geometry=geometry), fill=colors[6], color=colors[6], size=1) +
  ylim(c(48.8, 48.92)) +
  theme_void() +
  theme(plot.margin = margin(-1, 0, 0, 0, unit="cm"))

perthP <- ggplot() +
  geom_sf(data=perth, aes(geometry=geometry), fill=colors[7], color=colors[7], size=1) +
  theme_void()
...
PlotGrid <- plot_grid(amsterdamP, chicagoP, laP,
          milanP, nycP,parisP,
          perthP, singaporeP, torontoP, nrow=3, ncol=3) +
...
ggdraw(PlotGrid) +
  ...
  draw_label(label="MILAN", x=0.15, y=0.568, size=fontsize, fontfamily = font1, fontface = "bold", color = colors[4]) +
  draw_label(label="NEW YORK CITY", x=0.5, y=0.568, size=fontsize, fontfamily = font1, fontface = "bold", color = colors[5]) +
  draw_label(label="PARIS", x=0.85, y=0.568, size=fontsize, fontfamily = font1, fontface = "bold", color = colors[6]) +
  draw_label(label="PERTH", x=0.15, y=0.268, size=fontsize, fontfamily = font1, fontface = "bold", color = colors[7]) +
